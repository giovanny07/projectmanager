...
window.ProjectManager.loadPredecessorSelect = function (opts) {
    ...
    // Patrón splitcat/gantt: URL hardcodeada con root_doc + /plugins/<nombre>/
    const url = `${CFG_GLPI.root_doc}/plugins/projectmanager/ajax/get_project_tasks.php`
              + `?projects_id=${opts.projectId}&exclude_task_id=${opts.currentTaskId}`;

    console.log('ProjectManager AJAX URL:', url);

    fetch(url, { method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' } })

    fetch(url, { method: 'GET', headers: { 'X-Requested-With': 'XMLHttpRequest' } })
    .then(r => {
        console.log('ProjectManager AJAX status:', r.status);
        return r.ok ? r.json() : r.text().then(t => { console.error('Response body:', t); return Promise.reject(r.status); });
    })
    .then(data => {
